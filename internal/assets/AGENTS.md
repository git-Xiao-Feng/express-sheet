...
- `default_template.json` 的 `blocks` **必须用 8 种新 type**(`text_h` / `text_v` / `line_h` / `line_v` / `rect` / `barcode_h` / `barcode_v` / `qrcode`);旧 `text` / `barcode` 在 `internal/server/validate` 会被 400 拒绝(US-003)
